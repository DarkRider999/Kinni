...
function PlanBadge() {
  ...
  if (entitlement.plan === 'MASTER') {
    return <span className="rounded-full bg-amber-100 px-2.5 py-1 text-xs font-semibold text-amber-800 dark:bg-amber-900/40 dark:text-amber-200">Master · unlimited</span>;
  }
  if (entitlement.plan === 'PREMIUM') {
    return <span className="rounded-full bg-brand-100 px-2.5 py-1 text-xs font-semibold text-brand-800 dark:bg-brand-900/40 dark:text-brand-200">Premium</span>;
  }
  const left = entitlement.freeRunsRemaining ?? 0;
  return (
    <span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${left === 0 ? 'bg-red-100 text-red-800 dark:bg-red-900/40 dark:text-red-200' : 'bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-200'}`}>
      {left} of {entitlement.freeRunsLimit} free left
    </span>
  );
}
          ...
          {ready && user ? (
            <>
              ...
              {entitlement?.plan === 'FREE' && (
                <button type="button" className="btn-primary !px-3" onClick={onUpgrade}>Upgrade</button>
              )}
              ...
              <button type="button" className="btn-ghost !px-3" onClick={logout}>Sign out</button>
            </>
          ) : (
